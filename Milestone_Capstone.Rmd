...
library(stringr)
library(data.table)
Path <- "N:/R Courses/CapstoneProject_JHK"
```
...
```{r}
# Plot 1-grams
FreqTokTwitter %>% top_n(20) %>% as.data.frame() %>%
  ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
  labs(x = "1-Gram",y="Frequency")

FreqTokBlogs %>% top_n(20) %>% as.data.frame() %>%
  ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
  labs(x = "1-Gram",y="Frequency")

FreqTokNews %>% top_n(20) %>% as.data.frame() %>%
  ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
  labs(x = "1-Gram",y="Frequency")

#Plot 2-grams
FreqTokTwoGramTwitter %>% top_n(20) %>% as.data.frame() %>%
  ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
  labs(x = "2-Gram",y="Frequency")

FreqTokTwoGramBlogs %>% top_n(20) %>% as.data.frame() %>%
  ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
  labs(x = "2-Gram",y="Frequency")

FreqTokTwoGramNews %>% top_n(20) %>% as.data.frame() %>%
  ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
  labs(x = "2-Gram",y="Frequency")

#Plot 3-grams
FreqTokThreeGramTwitter %>% top_n(20) %>% as.data.frame() %>%
    ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
        labs(x = "3-Gram",y="Frequency")

FreqTokThreeGramBlogs %>% top_n(20) %>% as.data.frame() %>%
    ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
        labs(x = "3-Gram",y="Frequency")

FreqTokThreeGramNews %>% top_n(20) %>% as.data.frame() %>%
    ggplot( aes(reorder(V1,-N), N))+ geom_col()+
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+ 
        labs(x = "3-Gram",y="Frequency")
